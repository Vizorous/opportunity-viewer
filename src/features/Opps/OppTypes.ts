...
export const dataSchema = Joi.array().items(Joi.object({
    ...
    }).allow(null).empty(null).default({
        company: {
            name: "Default Company",
        }
    }),
    cover_photo: Joi.object({
        url: Joi.string().uri().allow('', null).empty(['', null]).default('https://s3.eu-west-1.amazonaws.com/gis-production-eu/7o7Fw4Zs994VT3BTfvMeh4XL'),
    }).allow(null).empty(null).default({
        url: "https://cdn-expa.aiesec.org/gis-img/gt_default.png",
    }),
    programme: Joi.object({
        short_name_display: Joi.string().allow('', null).empty(['', null]).empty(Joi.not('GV', "GTa", "GTe")).default('GTa'),
    }).allow(null).empty(null).default({
